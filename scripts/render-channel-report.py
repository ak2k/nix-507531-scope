...
# ///
"""
Assemble the final per-channel REPORT.md by splicing Tier 2 and Tier 3
markdown fragments into the Tier 1 report produced by aggregate-scan.py.
...
section is simply omitted (useful when Tier 3 eval is skipped, e.g. during
local iteration).

Also extends the top summary table to include per-tier counts when the
...
def extend_top_summary_table(
    ...
    """Add two rows to the top "Summary" table with Tier 2/3 headline counts.

    The Tier 1 table ends with an explicit count row (e.g. 'Other signature-
    ...
    if tier2:
        extra_rows.append(
            f"| Tier 2 — binaries linking a failing dylib | "
            f"{tier2.get('distinct_binaries', 0)} |"
        )
        extra_rows.append(
            f"| Tier 2 — distinct packages | "
            f"{tier2.get('distinct_dependent_packages', 0)} |"
        )
    if tier3:
        extra_rows.append(
            f"| Tier 3 — packages directly declaring a failing build input "
    ...
    # Anchor: last row of the Tier-1 summary table, which begins with
...
def main(argv: list[str]) -> int:
    ...
    p.add_argument("--report", type=Path, required=True, help="Input Tier-1 REPORT.md (edited in place).")
    p.add_argument(
        "--tier2-section",
        type=Path,
        default=None,
        help="Tier 2 markdown fragment (from compute-load-time-dependents.py --out-section).",
    )
    p.add_argument(
        "--tier3-section",
        type=Path,
        default=None,
        help="Tier 3 markdown fragment (from compute-build-time-dependents.py --out-section).",
    )
    p.add_argument(
        "--tier2-summary",
        type=Path,
        default=None,
        help="Tier 2 summary JSON, used to extend the top-summary table.",
    )
    p.add_argument(
        "--tier3-summary",
        type=Path,
        default=None,
        help="Tier 3 summary JSON, used to extend the top-summary table.",
    )
