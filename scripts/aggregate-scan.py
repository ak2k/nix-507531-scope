...
Classification scheme (per scanned Mach-O slice):
  ...
  page_hash_mismatch    status=ok, has_code_signature, n_mismatches > 0
                        → the nixpkgs#507531 bug proper. Always in the
...
def aggregate(input_path: Path):
    ...
    buckets_by_arch: dict[str, collections.Counter] = collections.defaultdict(collections.Counter)
    ...
    buckets_by_kind: dict[str, collections.Counter] = collections.defaultdict(collections.Counter)
    ...
    for r in load(input_path):
        ...
        for s in r.get("slices", []):
            ...
            if b == "page_hash_mismatch":
                ...
                error_samples.setdefault(key, {"store_path": sp, "path": s.get("path"), "error": s.get("error")})
...
def render_markdown(agg: dict, channel_label: str) -> str:
    ...
    # Header
    lines.append(f"# nixpkgs#507531 cache scan — {channel_label}")
    lines.append("")
    lines.append(
    ...
    lines.append(f"| Mach-O slices parsed | {_fmt_n(sum(agg['slices_by_arch'].values()))} |")
    lines.append(f"| Page-hash mismatches (slices) | {_fmt_n(page_hash_slices)} |")
    lines.append(f"| Page-hash mismatches (distinct packages) | {_fmt_n(page_hash_pkgs)} |")
    ...
    lines.append(f"| Other signature-invalid (distinct packages) | {_fmt_n(other_pkgs)} |")
    lines.append("")
    ...
    lines.append("")
    lines.append("| Arch | Slices scanned | Page-hash mismatch | Other sig-invalid | Clean (signed) | Unsigned | Noise |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|")
    for arch_label, bc in _collapse_arches(agg["slices_by_arch"]):
        total = bc.get("page_hash_mismatch", 0) + bc.get("other_sig_invalid", 0) + \
                bc.get("clean", 0) + bc.get("unsigned", 0) + \
                bc.get("not_real_macho", 0) + bc.get("scanner_error", 0)
        lines.append(
            "| {}{}{} | {} | {} | {} | {} | {} | {} |".format(
    ...
    fat_c = bk.get("fat", collections.Counter()) if isinstance(bk.get("fat"), dict) else collections.Counter(bk.get("fat", {}))
    thin_c = bk.get("thin", collections.Counter()) if isinstance(bk.get("thin"), dict) else collections.Counter(bk.get("thin", {}))
    lines.append("## Fat vs thin Mach-O")
    lines.append("")
    lines.append("| Kind | Slices | Page-hash mismatch | Other sig-invalid | Clean (signed) | Unsigned |")
    lines.append("|---|---:|---:|---:|---:|---:|")
    ...
    # Failing packages table
    if page_hash_pkgs:
        lines.append("## Failing packages (page-hash mismatch)")
        lines.append("")
        lines.append("Sorted by number of failing slices.")
        lines.append("")
        lines.append("| Package | Failing slices | Store path |")
        lines.append("|---|---:|---|")
        for sp, n in sorted(agg["pkg_page_hash"].items(), key=lambda x: (-x[1], x[0])):
    ...
    if other_pkgs:
        ...
        lines.append(
            ...
            " Mechanism may or may not be the same as nixpkgs#507531."
        )
        lines.append("")
        ...
        for sp, rows in sorted(by_pkg.items(), key=lambda x: (-len(x[1]), x[0])):
            err_kinds = sorted({(r.get("error") or "").split(":")[0].strip() for r in rows})
            lines.append(f"| {_pkg_name(sp)} | {len(rows)} | {', '.join(err_kinds)} | `{sp}` |")
        lines.append("")
    ...
    lines.append("|---|---:|")
    lines.append(f"| `page_hash_mismatch` | {_fmt_n(buckets.get('page_hash_mismatch', 0))} |")
    lines.append(f"| `other_sig_invalid` | {_fmt_n(buckets.get('other_sig_invalid', 0))} |")
    lines.append(f"| `clean` (signed, verified) | {_fmt_n(buckets.get('clean', 0))} |")
    lines.append(f"| `unsigned` (Mach-O without LC_CODE_SIGNATURE) | {_fmt_n(buckets.get('unsigned', 0))} |")
    lines.append(f"| `not_real_macho` (Java .class, PPC big-endian, etc.) | {_fmt_n(buckets.get('not_real_macho', 0))} |")
    ...
    lines.append("- Per path: `<hash>.narinfo` → stream NAR over HTTP → decompress (xz/zstd/bz2)"
                 " inline → walk entries, no on-disk NAR persistence.")
    lines.append("- Per regular file: peek 4 bytes; buffer and analyze only if Mach-O magic matches.")
    lines.append("- Per Mach-O slice (thin or fat): parse `LC_CODE_SIGNATURE`, find the SHA-256"
                 " CodeDirectory, recompute per-page SHA-256 over"
                 " `data[i*ps : min((i+1)*ps, code_limit)]`, compare against the stored hash slot.")
    lines.append("- `page_hash_mismatch` is defined as: at least one computed per-page SHA-256"
    ...
    lines.append("- `other_sig_invalid` is defined as: LC_CODE_SIGNATURE is present but the"
...
def write_failing_csv(path: Path, rows: list[dict]) -> None:
    if not rows:
        return
    fieldnames = [
        "store_path", "path", "arch", "size", "linker_signed", "cd_flags",
        "page_size", "n_code_slots", "code_limit", "n_mismatches",
        "first_bad_page", "first_bad_offset",
    ]
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--summary-json", default="summary.json", help="JSON summary output path")
    p.add_argument("--failing-csv", default="",
                   help="Optional CSV of failing slices (page-hash mismatches)")
    p.add_argument("--channel-label", default="nixpkgs-25.11-darwin",
...
def main() -> int:
    ...
    print(f"Page-hash mismatch slices:  {b.get('page_hash_mismatch', 0)}  ({len(agg['pkg_page_hash'])} packages)")
    print(f"Other sig-invalid slices:   {b.get('other_sig_invalid', 0)}  ({len(agg['pkg_other_sig'])} packages)")
    ...
    print(f"\nWrote: {args.out}, {args.summary_json}" + (f", {args.failing_csv}" if args.failing_csv else ""))
    return 0
