...
This is the empirical blast-radius check for nixpkgs#507531 / NixOS/nix#15638:
...
class AsyncByteStream:
    ...
    async def read(self, n: int) -> bytes:
        ...
        if len(self._buf) - self._off < n:
            raise EOFError(f"unexpected eof: need {n}, have {len(self._buf) - self._off}")
            ...
            if size == 0:
                ...
                if _macho_magic(head):
                    if size <= max_macho_bytes:
                        rest = await stream.read(size - probe_len) if size > probe_len else b""
                        content = head + rest
                    else:
...
def _analyze_fat(data: bytes, is_64: bool) -> list[SliceReport]:
    ...
    if nfat == 0 or nfat > 16:
        return [SliceReport(True, "unknown", 0, 0, error=f"implausible fat nfat={nfat}")]
    ...
    for i in range(nfat):
        ...
        if offset + size > len(data):
            results.append(
                SliceReport(True, _cputype_name(cputype, cpusubtype), cputype, cpusubtype,
...
def _analyze_thin(data: bytes) -> SliceReport:
    ...
    try:
        if is_64:
            _m, cputype, cpusubtype, _ft, ncmds, sizeofcmds, _flags, _res = struct.unpack_from(
                "<IIIIIIII", data, 0
            )
            hdr_size = 32
        else:
            _m, cputype, cpusubtype, _ft, ncmds, sizeofcmds, _flags = struct.unpack_from(
                "<IIIIIII", data, 0
            )
            hdr_size = 28
    except struct.error:
    ...
    for i in range(sb_count):
        ...
        try:
            (
                _cd_magic, cd_len, _cd_ver, cd_flags, hash_off, _ident_off,
                n_special, n_code, code_limit,
                hash_size, hash_type, _platform, page_size_log2, _spare2,
            ) = struct.unpack_from(">IIIIIIIIIBBBBI", blob, eoff)
        except struct.error:
            continue
        cand = {
            "eoff": eoff, "cd_flags": cd_flags, "hash_off": hash_off,
        ...
        if best is None or (best["hash_type"] != CS_HASHTYPE_SHA256 and hash_type == CS_HASHTYPE_SHA256):
            best = cand
    ...
    mismatches: list[int] = []
    for i in range(n):
        expected = blob[hash_off_in_blob + i * hash_size : hash_off_in_blob + (i + 1) * hash_size]
        page_start = i * ps
...
def load_done(conn: sqlite3.Connection) -> set[str]:
    return {r[0] for r in conn.execute("SELECT store_path FROM scanned")}
...
async def scan_store_path(
    ...
    try:
        ...
        async with client.stream("GET", url) as response:
            ...
            async for entry in iter_nar_regular_files(stream, max_macho_bytes=max_macho_bytes):
                ...
                except Exception as e:  # defensive: don't let one bad file kill the scan
                    slices_out.append(
                        {
                            "store_path": store_path,
                            "path": entry.path,
...
def _persist_result(
    ...
    if counters.done % log_every == 0 or counters.done == counters.total:
        ...
        log.info(
            ...
            counters.done, counters.total, rate, eta_s / 60,
            counters.macho_slices, counters.failing_slices,
            counters.no_narinfo, counters.fetch_errors, counters.parse_errors,
        )
...
def _run_mp(args: argparse.Namespace) -> None:
    ...
    log.info(
        ...
        len(todo), args.workers, args.batch_size, args.per_worker_concurrency,
    )
    ...
    log.info(
        ...
        counters.done, counters.macho_slices, counters.failing_slices,
        counters.no_narinfo, counters.fetch_errors, counters.parse_errors,
    )
        ...
        async def worker(sp: str) -> None:
            ...
            async with sem:
                if stopping:
                    return
                try:
                    result = await scan_store_path(client, args.cache, sp, args.max_macho_bytes)
                except Exception as e:
                    result = {
                        "store_path": sp,
                        "status": "fetch_error",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(
        description="Scan an aarch64-darwin nixpkgs channel's cache for the nixpkgs#507531 page-hash bug."
    )
    ...
    p.add_argument("--state", default="./darwin-cache-scan.state.db",
                   help="SQLite state DB for resume (default: ./darwin-cache-scan.state.db)")
    p.add_argument("--out", default="./darwin-cache-scan.jsonl",
    ...
    p.add_argument("--per-worker-concurrency", type=int, default=32,
    ...
    p.add_argument("--max-macho-bytes", type=int, default=2 << 30,
