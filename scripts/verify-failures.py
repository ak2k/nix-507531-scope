...
def realize(store_path: str, cache_url: str) -> tuple[bool, str]:
    ...
    if r.returncode != 0:
        return False, (r.stderr or r.stdout).strip().splitlines()[-1] if (r.stderr or r.stdout).strip() else "realize failed"
    return True, ""
...
def _auto_verifier() -> list[str]:
    ...
    if rc:
        return [rc, "verify"]
    print("no signature verifier found (need /usr/bin/codesign on darwin or "
...
def main() -> int:
    ...
    p.add_argument("--verifier", nargs="+", default=None,
    ...
    print(f"verifying {len(rows)} slices from {len({r['store_path'] for r in rows})} unique packages", file=sys.stderr)
    ...
    for i, sp in enumerate(unique_paths, 1):
        ...
        if ok:
            realized_ok.add(sp)
        print(f"  [{i}/{len(unique_paths)}] {'OK ' if ok else 'FAIL'} {sp}  {msg if not ok else ''}", file=sys.stderr)

    # Run verifier on each row
    agreement = {"agree_fail": 0, "disagree_scanner_false_positive": 0, "realize_failed": 0}
    results: list[dict] = []
    ...
    for i, r in enumerate(rows, 1):
        ...
        if sp not in realized_ok:
            r2 = {**r, "verifier_exit": None, "verifier_message": "realize failed",
                  "verdict": "realize_failed"}
            agreement["realize_failed"] += 1
            results.append(r2)
            continue
        if not os.path.exists(full_path):
            r2 = {**r, "verifier_exit": None, "verifier_message": "file not found in realized store path",
        ...
        results.append(r2)
        print(f"  [{i}/{len(rows)}] {verdict}  {r['path']}  (rc={code})", file=sys.stderr)
    ...
    md.append("")
    md.append(f"Verified every slice the scanner flagged as failing against an")
    md.append(f"independent signature verifier: `{' '.join(verifier)}`.")
    md.append("")
    md.append("| Outcome | Count |")
    md.append("|---|---:|")
    md.append(f"| Scanner failing, verifier fails (agreement) | {agreement['agree_fail']} |")
    md.append(f"| Scanner failing, verifier passes (disagreement — possible false positive) | {agreement['disagree_scanner_false_positive']} |")
    md.append(f"| Realize failed (could not fetch package from cache) | {agreement['realize_failed']} |")
    md.append(f"| **Total slices verified** | **{total}** |")
    md.append("")
    disagreements = [r for r in results if r["verdict"] == "disagree_scanner_false_positive"]
    if disagreements:
        ...
        for r in disagreements:
            pkg = r["store_path"].rsplit("-", 1)[-1] if "-" in r["store_path"] else r["store_path"]
            md.append(f"| {pkg} | `{r['path']}` | {r['arch']} | {r.get('scanner_detail', r.get('error', ''))} | `{r['verifier_message']}` |")
        md.append("")
    else:
        md.append("No disagreements: every scanner-flagged failure was independently confirmed by the verifier.")
        md.append("")
